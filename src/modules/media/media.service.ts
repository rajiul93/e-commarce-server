...
const uploadImage = async (
  ...
  try {
    return await saveImageToDb(file.originalname, url, r2Key, size, alt, useCase, userId);
  } catch (err) {
...
const updateImage = async (
  ...
  if (!oldImage) {
    await deleteFromR2(newR2Key).catch(() => undefined);
    throw new AppError(MEDIA_ERRORS.IMAGE_NOT_FOUND, httpStatus.NOT_FOUND);
  }
  ...
  await deleteFromR2(oldR2Key).catch(() => undefined);
